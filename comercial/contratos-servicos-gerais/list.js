(function () {
  ...
  var COLLECTION_NAME = "commercial_general_service_contracts";
  ...
  function buildPublicContractUrl(contractId) {
    var url = new URL("/comercial/contratos-servicos-gerais/editor.html", window.location.origin);
  ...
  function createBlankPublicContractData(contractId, publicLink, timestamp) {
    return {
      contract: {
        contractCode: "GERAIS-" + contractId.slice(0, 8).toUpperCase(),
  ...
  async function importContractFromFile(file) {
    ...
    try {
      ...
      await docRef.set({
        archived: false,
        contractId: docRef.id,
        source: "general-service-contract-upload",
        createdAt: timestamp,
        updatedAt: timestamp,
  ...
  async function deleteContract(id) {
    if (!canManageContracts()) return;
    var confirmed = window.confirm("Arquivar este contrato de servicos gerais? O registro continuara preservado.");
    if (!confirmed) return;
    var db = initializeFirebase();
    var user = currentPortalUser;
  ...
  function buildCard(doc, isArchived) {
    ...
    var actions = "";
    if (!isArchived) {
      actions += '<a class="button" href="/comercial/contratos-servicos-gerais/editor.html?id=' + encodedId + '">Abrir contrato</a>';
      actions += '<button class="button secondary" type="button" data-public-link="' + encodedId + '">Ver Previa</button>';
      if (canManageContracts()) {
        actions += '<button class="button warning" type="button" data-archive="' + doc.id + '">Arquivar</button>';
      }
    } else {
      if (canManageContracts()) {
        actions += '<a class="button secondary" href="/comercial/contratos-servicos-gerais/editor.html?id=' + encodedId + '">Ver contrato</a>';
    ...
    return (
      ...
      "</div>" +
      '<span class="chip">Serviços Gerais</span>' +
      "</div>" +
      '<div class="meta-grid">' +
  ...
  loadContracts().catch(function (error) {
    ...
    renderMessage(listElement, "error-state", "Nao foi possivel carregar a base de servicos gerais.");
    console.error(error);
  });
})();
