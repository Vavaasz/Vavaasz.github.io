(function () {
  ...
  var COLLECTION_NAME = "commercial_contracts";
  ...
  function buildPublicContractUrl(contractId) {
    var url = new URL("/comercial/contratos/editor.html", window.location.origin);
  ...
  function createBlankPublicContractData(contractId, publicLink, timestamp) {
    return {
      contract: {
        contractCode: "PUBLIC-" + contractId.slice(0, 8).toUpperCase(),
  ...
  async function importContractFromFile(file) {
    ...
    try {
      ...
      await docRef.set({
        archived: false,
        contractId: docRef.id,
        source: "commercial-contract-upload",
        createdAt: timestamp,
        updatedAt: timestamp,
  ...
  async function deleteContract(id) {
    if (!canManageContracts()) return;
    var confirmed = window.confirm("Arquivar este contrato de monitoramento? O registro continuara preservado.");
    if (!confirmed) return;
    var db = initializeFirebase();
    var user = currentPortalUser;
  ...
  function buildCard(doc, isArchived) {
    ...
    var monitored = text(service.monitoredLocationName || service.monitoredReference || service.monitoredAddress) || "Local monitorado nao informado";
    ...
    var actions = "";
    if (!isArchived) {
      actions += '<a class="button" href="/comercial/contratos/editor.html?id=' + encodedId + '">Abrir contrato</a>';
      ...
      if (canManageContracts()) {
        actions += '<a class="button secondary" href="/comercial/contratos/editor.html?id=' + encodedId + '">Ver contrato</a>';
    ...
    return (
      ...
      "</div>" +
      '<span class="chip">Monitoramento</span>' +
      "</div>" +
      '<div class="meta-grid">' +
      ...
      createMetaItem("Monitoramento", formatMoney(pricing.monitoringMonthly)) +
      createMetaItem("Locacao", formatMoney(pricing.equipmentRentalMonthly)) +
  ...
  loadContracts().catch(function (error) {
    ...
    renderMessage(listElement, "error-state", "Nao foi possivel carregar a base de monitoramento.");
    console.error(error);
  });
})();
