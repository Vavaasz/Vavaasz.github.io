const COLLECTION_NAME = "commercial_specialized_service_contracts";
const PORTAL_SESSION_DATA_KEY = "portal_gate_user";
const PORTAL_PERSIST_DATA_KEY = "portal_gate_user_persist";
const WHATSAPP_MESSAGE = "Segue o contrato de serviços especializados TKA em PDF.";
const CONTRACT_TITLE = "Contrato de Prestacao de Servicos Especializados";
const PAGE = {
  width: 595.28,
  height: 841.89,
  top: 118,
...
const DEFAULT_EQUIPMENT = [
  { quantity: "01", description: "Vigia Desarmado", location: "Escala a definir", value: "" },
  { quantity: "01", description: "Vigilante Armado", location: "Escala a definir", value: "" }
];
...
function buildDefaultContractContent() {
  ...
  const serviceType = text(state.service.communicationMethod) || "seguranca patrimonial, vigilancia, vigia ou servico especializado";
  ...
  return {
    ...
    intro: [
      ...
      `${TKA_PROFILE.legalName}, com sede na ${TKA_PROFILE.address}, nº ${TKA_PROFILE.number}, ${TKA_PROFILE.district} - ${TKA_PROFILE.city}/${TKA_PROFILE.state}, inscrita no CNPJ sob o nº ${TKA_PROFILE.cnpj}, portadora do Certificado de Seguranca expedido pela Policia Federal nº ${TKA_PROFILE.certificate}, neste ato representada por ${TKA_PROFILE.representativeName}, portador do RG nº ${TKA_PROFILE.representativeRg} e inscrito no CPF/MF nº ${TKA_PROFILE.representativeCpf}, doravante denominada simplesmente CONTRATADA.`,
      "As partes resolvem celebrar contrato de prestacao de servicos especializados de seguranca patrimonial, vigilancia, vigia e atividades correlatas, mediante as clausulas e condicoes abaixo."
    ],
    definitions: [
      {
        label: "SERVICOS ESPECIALIZADOS",
        text: `Servicos de ${serviceType}, executados por profissionais alocados conforme os postos, escalas e horarios definidos neste contrato.`
      },
      {
    ...
    clauses: [
      {
        ...
        paragraphs: [
          `1.1. A CONTRATADA, sob sua exclusiva responsabilidade e dentro da melhor probidade, diligencia e zelo, compromete-se a prestar servicos de ${serviceType} nas dependencias da CONTRATANTE, situadas em ${serviceAddress || "endereco nao informado"}.`,
          "1.2. A prestacao dos servicos sera composta pelo efetivo descrito no Anexo I, observando as escalas, horarios e postos de trabalho definidos entre as partes.",
          "1.3. Na hipotese de aumento, reducao ou modificacao dos servicos contratados, as partes deverao formalizar aditivo com as novas condicoes e valores."
        ]
      },
      {
        title: "Clausula Segunda - Prestacao dos Servicos",
        paragraphs: [
          "2.1. O servico consiste na guarda, protecao e seguranca das instalacoes, com rondas, controle visual, observacao de entrada e saida de pessoas ou veiculos e atuacao preventiva contra ameacas, roubos, atos de violencia e outras infracoes a ordem e seguranca.",
          "2.2. Quando constatada anormalidade, a CONTRATADA podera comunicar a base operacional, responsaveis indicados pela CONTRATANTE, equipe de pronta resposta, forca policial ou servico de socorro emergencial, conforme a ocorrencia.",
          "2.3. Servicos adicionais nao descritos no objeto deverao ser orcados a parte e aprovados por escrito antes da execucao."
        ]
      },
      {
        title: "Clausula Terceira - Preco, Hora Extra e Reajuste",
        paragraphs: [
          `3.1. Pela prestacao dos servicos, a CONTRATANTE pagara a CONTRATADA o valor mensal de ${monthlyValue}, com vencimento ${dueDay}.`,
          `3.2. Horas excedentes solicitadas pela CONTRATANTE serao cobradas pelo valor de ${extraHourValue} por hora/homem, observada ${minimumExtra}.`,
          `3.3. O pagamento sera realizado por ${paymentMethod}, juntamente com os documentos fiscais aplicaveis.`,
          `3.4. O valor sera reajustado ${reajust}, considerando convencao coletiva, encargos trabalhistas, custos operacionais e indices aplicaveis.`
        ]
      },
      {
        title: "Clausula Quarta - Responsabilidades da Contratada",
        paragraphs: [
          "4.1. A CONTRATADA responsabiliza-se pela administracao, supervisao, substituicao e orientacao dos profissionais alocados.",
          "4.2. A CONTRATADA arcara com salarios, encargos sociais, previdenciarios, trabalhistas, fiscais, uniformes e demais obrigacoes relacionadas aos seus empregados.",
          "4.3. A CONTRATADA devera manter equipe apta ao desempenho das funcoes, observando postura, apresentacao, registro de ocorrencias e cumprimento das normas aplicaveis ao servico."
        ]
      },
      {
        title: "Clausula Quinta - Responsabilidades da Contratante",
        paragraphs: [
          "5.1. A CONTRATANTE fornecera condicoes adequadas para execucao dos servicos, incluindo acesso ao local, normas internas, contatos responsaveis, orientacoes operacionais e infraestrutura minima.",
          "5.2. A CONTRATANTE comunicara previamente qualquer necessidade de reforco, alteracao de escala, ampliacao de jornada ou servico extraordinario.",
          "5.3. E vedada a subordinacao direta dos profissionais da CONTRATADA pela CONTRATANTE, preservando-se a natureza civil e empresarial deste contrato."
        ]
      },
      {
        ...
        paragraphs: [
          ...
          "6.2. O contrato podera ser rescindido mediante aviso previo de 30 dias, sem prejuizo da cobranca de valores vencidos, horas extras, servicos extraordinarios e eventuais multas pactuadas.",
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "7.2. As partes declaram que este contrato nao cria vinculo empregaticio entre a CONTRATANTE e os empregados, prepostos ou colaboradores da CONTRATADA.",
...
function renderPreview() {
  ...
  html += `
    <article class="preview-card">
      <h3>Anexo I - Efetivo, Funcoes e Escalas</h3>
      <table class="annex-table">
        ...
        <tbody>
          ...
          `).join("") : `
            <tr>
              <td colspan="4">Nenhum posto informado.</td>
            </tr>
          `}
        </tbody>
      </table>
...
function sanitizeRecordForCloud() {
  ...
  return {
    ...
    source: "specialized-service-contract-editor",
...
async function ensurePdfAssets() {
  ...
  if (!pdfAssetsPromise) {
    pdfAssetsPromise = Promise.all([
      loadAssetAsDataUrl("/comercial/contratos-servicos-especializados/assets/TKA-logo.jpeg"),
      loadAssetAsDataUrl("/comercial/contratos-servicos-especializados/assets/letterhead.png")
...
async function generatePdfDocument(options = {}) {
  ...
  try {
    ...
    function drawPageFrame() {
      ...
      doc.text("Contrato - Serviços Especializados TKA", 555, 52, { align: "right" });
      doc.setFont("helvetica", "normal");
      doc.setFontSize(9);
    ...
    function drawSignatureBlock() {
      ...
      function drawOneSignature(x, label, signerName, dataUrl, placeholder) {
        ...
        if (dataUrl) {
          ...
          doc.text(placeholder || "Assinatura", x + signatureWidth / 2, signatureY + 56, {
            align: "center",
        ...
        doc.setFontSize(8.5);
        doc.setTextColor(47, 36, 31);
        doc.text(String(signerName || "Assinatura").slice(0, 70), x + signatureWidth / 2, signatureY + 84, {
          align: "center",
          maxWidth: signatureWidth - 24
        });
      }
    ...
    drawPageFrame();
    drawSectionTitle("Resumo de Serviços Especializados");
    drawFieldGrid([
    ...
    drawSectionTitle("Anexo I - Efetivo, Funcoes e Escalas");
    if (content.equipment.length) {
      ...
      drawParagraph("Nenhum posto informado no anexo.");
    }
  ...
  try {
    if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
      await navigator.share({
        title: "Contrato - Serviços Especializados TKA",
...
function buildPublicContractUrl(contractId) {
  ...
  const url = new URL("/comercial/contratos-servicos-especializados/editor.html", window.location.origin);
...
async function loadData() {
  ...
  if (loaded) {
    consumeImportNotice();
    updateSaveStatus("contrato de serviços especializados carregado e pronto para edição.");
    return;
  }
  if (requestedId) {
    elements.exportStatus.textContent = "Contrato não encontrado na Base de serviços especializados. Um modelo novo foi aberto.";
  }
  updateSaveStatus("Modelo de serviços especializados pronto para preenchimento.");
}
