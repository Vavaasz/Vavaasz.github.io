(function () {
  ...
  var COLLECTION_NAME = "commercial_intermittent_contracts";
  var EXAMPLE_PDF_PATH = "/comercial/contratos-intermitente/assets/contrato-intermitente-exemplo-tka-2025.pdf";
  var LEGAL_EDITOR_EMAILS = new Set([
  ...
  function canManageContracts() {
    var email = text(currentPortalUser && currentPortalUser.email).toLowerCase();
    return Boolean(currentPortalUser && (currentPortalUser.permissions?.admin || LEGAL_EDITOR_EMAILS.has(email)));
  }
  ...
  function buildPublicContractUrl(contractId) {
    var url = new URL("/comercial/contratos-intermitente/editor.html", window.location.origin);
  ...
  function createBlankPublicContractData(contractId, publicLink, timestamp) {
    return {
      contract: {
        contractCode: "INTER-" + contractId.slice(0, 8).toUpperCase(),
  ...
  async function copyPublicLink(contractId, button) {
    ...
    try {
      ...
      window.prompt("Copie o link de preenchimento do empregado:", link);
    }
  }

  async function copyExamplePdfLink(button) {
    var link = new URL(EXAMPLE_PDF_PATH, window.location.origin).toString();
    try {
      var copied = await copyTextToClipboard(link);
      if (!copied) throw new Error("Clipboard unavailable");
      markPublicLinkCopied(button);
      setNewPublicLinkStatus("Link do exemplo copiado. Envie este PDF ao empregado como referencia.");
    } catch (error) {
      window.prompt("Copie o link do exemplo:", link);
      setNewPublicLinkStatus("Link do exemplo gerado.");
    }
  }
  ...
  async function importContractFromFile(file) {
    ...
    try {
      ...
      await docRef.set({
        archived: false,
        contractId: docRef.id,
        source: "intermittent-contract-upload",
        createdAt: timestamp,
        updatedAt: timestamp,
  ...
  async function createNewPublicLink() {
    ...
    setNewPublicLinkStatus("Criando link publico para o empregado preencher...");
    var publicLink = "";
    ...
    try {
      ...
      if (!copied) {
        window.prompt("Copie o novo link publico de preenchimento:", publicLink);
        setNewPublicLinkStatus("Novo link publico de preenchimento gerado.");
      } else {
        setNewPublicLinkStatus("Novo link publico copiado. Envie para o empregado preencher e assinar.");
      }

      setNewPublicLinkButtonState(false, "Link copiado");
      await loadContracts();
      setTimeout(function () {
        setNewPublicLinkButtonState(false, "Novo Link Empregado");
      }, 1800);
    } catch (error) {
      console.error(error);
      if (publicLink) {
  ...
  var copyExamplePdfBtn = document.getElementById("copyExamplePdfBtn");
  ...
  if (copyExamplePdfBtn) {
    copyExamplePdfBtn.onclick = function () {
      copyExamplePdfLink(copyExamplePdfBtn).catch(function (error) {
        console.error(error);
        setNewPublicLinkStatus("Nao foi possivel copiar o link do exemplo.");
      });
    };
  }
  ...
  async function deleteContract(id) {
    if (!canManageContracts()) return;
    var confirmed = window.confirm("Arquivar este contrato de trabalho intermitente? O registro continuara preservado.");
    if (!confirmed) return;
    var db = initializeFirebase();
    var user = currentPortalUser;
  ...
  function buildCard(doc, isArchived) {
    ...
    var companyName = text(contractor.legalName || contractor.companyName) || "Contrato sem contratado";
    var cpf = text(contractor.cnpj) || "Nao informado";
    var monitored = text(service.monitoredLocationName || service.monitoredReference || service.monitoredAddress) || "Evento/base nao informado";
    ...
    var actions = "";
    if (!isArchived) {
      actions += '<a class="button" href="/comercial/contratos-intermitente/editor.html?id=' + encodedId + '">Abrir contrato</a>';
      actions += '<button class="button secondary" type="button" data-public-link="' + encodedId + '">Copiar link do empregado</button>';
      if (canManageContracts()) {
        actions += '<button class="button warning" type="button" data-archive="' + doc.id + '">Arquivar</button>';
      }
    } else {
      if (canManageContracts()) {
        actions += '<a class="button secondary" href="/comercial/contratos-intermitente/editor.html?id=' + encodedId + '">Ver contrato</a>';
    ...
    return (
      ...
      "<div>" +
      "<h2>" + companyName + "</h2>" +
      '<p class="muted">' + [monitored, [city, state].filter(Boolean).join(" / ")].filter(Boolean).join(" | ") + "</p>" +
      "</div>" +
      '<span class="chip">Trabalho Intermitente</span>' +
      "</div>" +
      '<div class="meta-grid">' +
      createMetaItem("ID do contrato", record.contractId || doc.id) +
      createMetaItem("CPF", cpf) +
      createMetaItem("Vigencia", period || "Nao informada") +
      createMetaItem("Diaria", formatMoney(pricing.monitoringMonthly)) +
      createMetaItem("Gratificacao", text(pricing.equipmentRentalMonthly) || "Nao informada") +
  ...
  loadContracts().catch(function (error) {
    ...
    renderMessage(listElement, "error-state", "Nao foi possivel carregar a base de trabalho intermitente.");
    console.error(error);
  });
})();
