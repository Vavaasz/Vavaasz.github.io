(function () {
  ...
  function readEquipmentForm() {
    return template.normalizeEquipment({
      id: el.equipmentId.value || template.newId(el.assetTag.value || "body-cam"),
  ...
  function fillEquipmentForm(equipment) {
    ...
    el.deviceCondition.value = data.deviceCondition || "Bom estado";
  ...
  function validateEquipment(equipment) {
    ...
    if (!equipment.assetTag) missing.push("patrimonio");
    if (!equipment.serialNumber) missing.push("numero de serie");
    if (!equipment.accessoriesDelivered.length) missing.push("acessorios");
  ...
  function saveEquipment(event) {
    ...
    db.collection(template.EQUIPMENT_COLLECTION).doc(equipment.id).set(payload, { merge: true }).then(function () {
      clearEquipmentForm();
      writeAuditLog(existing ? "update" : "create", equipment.assetTag || equipment.id, "Equipamento Body Cam salvo.", existing, payload);
    }).catch(function (error) {
      console.error(error);
      window.alert("Nao foi possivel salvar o equipamento.");
    });
  }
  ...
  function buildEquipmentCard(equipment) {
    return ''
      ...
      + '    <div><h2>' + escapeHtml([equipment.deviceBrand, equipment.deviceModel].filter(Boolean).join(" ") || "Body Cam") + '</h2>'
      + '    <p class="muted">Patrimonio ' + escapeHtml(equipment.assetTag || "-") + ' | Serie ' + escapeHtml(equipment.serialNumber || "-") + '</p></div>'
      + '    <div class="chip-row"><span class="chip">' + escapeHtml(equipment.active ? "Ativa" : "Inativa") + "</span></div>"
      + "  </div>"
      + '  <div class="meta-grid">'
      + createMetaItem("ID Interno", equipment.internalId || equipment.imeiOrSimNumber || "Nao informado")
      + createMetaItem("Acessorios", template.accessoriesLabel(equipment.accessoriesDelivered))
  ...
  function renderEquipment() {
    ...
    if (!rows.length) {
      empty(el.equipmentList, query ? "Nenhum equipamento encontrado." : "Nenhuma Body Cam cadastrada.");
      return;
    }
  ...
  function validateTermDraft(term) {
    var missing = [];
    if (!term.equipmentId) missing.push("equipamento");
    return missing;
  }
  ...
  function createTerm(event) {
    ...
    var id = template.newId("body-cam-term-" + draft.assetTag);
    draft.id = id;
    draft.publicLink = template.buildPublicLink(id);
    draft.source = "body-cam-dashboard";
    ...
    db.collection(template.TERMS_COLLECTION).doc(id).set(payload, { merge: true }).then(function () {
      ...
      writeAuditLog("create", draft.assignedUserEmail, "Termo Body Cam gerado para assinatura.", null, payload);
  ...
  function buildTermCard(term, archived) {
    ...
    var actions = ''
      + '<a class="button" href="/operacional/inventario/body-cam/editor.html?id=' + encodeURIComponent(term.id) + '">Abrir</a>'
    ...
    return ''
      ...
      + "  </div>"
      + '  <div class="meta-grid">'
      + createMetaItem("Equipamento", [term.deviceBrand, term.deviceModel].filter(Boolean).join(" ") || "Body Cam")
      + createMetaItem("Patrimonio", term.assetTag)
      + createMetaItem("Serie", term.serialNumber)
      + createMetaItem("ID Interno", term.internalId || term.imeiOrSimNumber)
  ...
  function updateTermArchive(id, archived) {
    ...
    return db.collection(template.TERMS_COLLECTION).doc(id).set(payload, { merge: true }).then(function () {
      writeAuditLog(archived ? "archive" : "restore", term.employeeName || id, archived ? "Termo Body Cam arquivado." : "Termo Body Cam reativado.", term, payload);
    });
  }
  ...
  function bindTermActions(container) {
    ...
    container.querySelectorAll("[data-export-term]").forEach(function (button) {
      button.onclick = function () {
        ...
        if (term) window.TKABodyCamPdf.download(withCurrentCompanySignature(term));
      };
    });
  ...
  function subscribeData() {
    db.collection(template.EQUIPMENT_COLLECTION).onSnapshot(function (snapshot) {
      ...
      console.error(error);
      empty(el.equipmentList, "Nao foi possivel carregar os equipamentos.", "error-state");
    });
