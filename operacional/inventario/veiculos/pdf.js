(function () {
  ...
  function safeFileName(value) {
    return template.cleanIdPart(value || "termo-veiculo-frota") || "termo-veiculo-frota";
  }
  ...
  function download(term) {
    ...
    drawMetaBox(doc, "CPF / MATRICULA", data.employeeDocument || data.employeeInternalId, PAGE.left + 204, y, 148, 34);
    ...
    y += 42;
    drawMetaBox(doc, "DATA DO TERMO", template.dateLabel(data.deliveryDate), PAGE.left, y, 130, 34);
    drawMetaBox(doc, "RESPONSAVEL PELA AUTORIZACAO", data.responsibleForDeliveryName, PAGE.left + 140, y, 383, 34);
    y += 42;
    ...
    drawMetaBox(doc, "DOCUMENTO BASE", template.MANUAL_LABEL, PAGE.left + 348, y, 175, 30);
    y += 38;

    y = drawEquipmentTable(doc, data, y);
    y = drawWrapped(doc, "Eu, " + (data.employeeName || "[NOME COMPLETO]") + ", portador do CPF n. " + (data.employeeDocument || "__________________________") + ", declaro estar autorizado a utilizar os veiculos da frota da empresa para o exercicio das minhas funcoes profissionais, assumindo total responsabilidade pelo seu uso sob as seguintes condicoes:", PAGE.left, y, PAGE.width - PAGE.left - PAGE.right, { size: 8.3, lineHeight: 9.2 });
    y += 4;
    ...
    doc.setFontSize(7.6);
    setColor(doc, RED);
    doc.text((data.acknowledgements.equipmentReceived ? "[x]" : "[ ]") + " Confirmo que estou autorizado a utilizar veiculos da frota da empresa.", PAGE.left, y);
    y += 11;
    doc.text((data.acknowledgements.policyRead ? "[x]" : "[ ]") + " Confirmo que li as normas de zelo, transito, danos, checklist, combustivel e KM.", PAGE.left, y);
    y += 18;
    ...
    drawSignatureImageBox(doc, "ASSINATURA DO DIRETOR", data.companySignerName || template.DEFAULT_COMPANY_SIGNER_NAME, data.companySignatureDataUrl, "Assinatura do Diretor", PAGE.left + signatureWidth + signatureGap, y, signatureWidth);
    y += 86;
    drawSignatureImageBox(doc, "COORDENACAO OPERACIONAL", data.coordinationSignerName || template.COORDINATION_SIGNATURE_LABEL, data.coordinationSignatureData, "Assinatura da Coordenacao Operacional", PAGE.left, y, signatureWidth);
    drawSignatureImageBox(doc, "LIDERANCA OPERACIONAL", data.leadershipSignerName || template.LEADERSHIP_SIGNATURE_LABEL, data.leadershipSignatureData, "Assinatura da Lideranca Operacional", PAGE.left + signatureWidth + signatureGap, y, signatureWidth);
    y += 86;
    ...
    drawInfoBox(doc, "RESPONSAVEL PELA AUTORIZACAO", data.responsibleForDeliveryName, PAGE.left + signatureWidth + signatureGap, y, signatureWidth);

    doc.setFont("helvetica", "normal");
    doc.setFontSize(6.8);
    setColor(doc, MUTED);
    doc.text("Tipo: " + template.TERM_TYPE + " | Versao: " + data.termVersion + " | Documento: " + template.MANUAL_LABEL, PAGE.left, PAGE.height - 22, { maxWidth: PAGE.width - PAGE.left - PAGE.right });

    doc.save(safeFileName("termo-veiculo-frota-" + (data.employeeName || data.assetTag || data.id)) + ".pdf");
  }

  window.TKAVehicleTermPdf = {
    download: download
  };
})();
